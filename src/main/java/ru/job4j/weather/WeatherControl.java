...
public class WeatherControl {
    ...
    public Mono<Weather> max() {
    	return weathers.all().sort((w1, w2) -> w1.getTemperature()-w2.getTemperature()).last();
    }
    
    @GetMapping(value = "/cityGreatThen/{temperature}")
    public Flux<Weather> cityGreatThen(@PathVariable Integer temperature) {
    	return weathers.all().filter(w -> w.getTemperature() > temperature);
    }
}
